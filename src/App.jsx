...
const BACKEND_URL = "https://llama-backend.onrender.com";
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div ref={chatWindowRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-900">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.sender === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] px-4 py-2 rounded-lg ${m.sender === "user" ? "bg-green-600" : "bg-gray-700"}`}>
                {m.type === "pdf" ? (
                  <div><p>📄 <b>{m.text}</b></p></div>
                ) : m.type === "image" ? (
                  <div><img src={m.url} alt="Uploaded" className="max-w-xs rounded mt-2" /></div>
                ) : (
                  <p>{m.text}</p>
                )}
              </div>
        ...
        <footer className="p-4 bg-gray-800 flex items-center space-x-2">
          ...
          />
          <button onClick={sendMessage} className="bg-green-600 px-4 py-2 rounded-lg hover:bg-green-700 transition">
            Send
          </button>
          <button onClick={() => fileInputRef.current.click()} className="bg-blue-500 px-4 py-2 rounded-lg hover:bg-blue-600 transition">
            Upload
          </button>
          <input
            type="file"
